...
class Graph:
    ...
    def hamilton(self, start):
        out = []
        out.append(self.vertices[start])
        v = start
        return self._hamilton(out, start, v)

    def _hamilton(self, out, start, v):
        for i in range(len(self.vertices)):
            if len(out) > len(self.vertices) + 1:
                break
            elif self.edges[self.edge_indices[v]][self.edge_indices[i]] > 0 and not i in out:
                out.append(i)
                out = self._hamilton(out, start, i)
        if len(out) >= len(self.vertices):
            if len(out) == len(self.vertices) and self.edges[self.edge_indices[v]][self.edge_indices[start]] > 0:
                out.append(start)
        else:
            out.pop()
        return(out)
